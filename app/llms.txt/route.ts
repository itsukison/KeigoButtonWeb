...
function build(): string {
  ...
  lines.push(
    `- [AI文章作成](${SITE_URL}/bunsho-sakusei-ai): 箇条書きのメモを、そのまま送れる仕事の文章に変えます。依頼・催促・お詫び・日程調整・報告などのメール本文とチャット連絡文が対象。**記事・ブログ・小説などの長文生成には対応しません**——既存文の編集とメモからの作成が範囲です。1日5回・300文字まで無料、登録不要。`,
  );
  lines.push(
